...
for _file in files:
    ...
    # output_dir = os.path.join(cwd, analysis_id)
    ...
    try:
        ...
        sys.exit('%s: Download object failed: %s' % (e, object_id))
    ...
    try:
        ...
        sys.exit('%s: Retrieve BAM header failed: %s' % (e, os.path.join(cwd, file_name)))
    ...
    # compare the RG ids
    if not rg_yaml == rg_bam: sys.exit('The read groups in metadata do not match with those in BAM!')  # die fast
    ...
    try:
        ...
        sys.exit('%s: RevertSam failed: %s' %(e, os.path.join(cwd, file_name)))
    ...
    if rg_replace: # need to replace
        for rg_old, rg_new in rg_replace.items():
            try:
                ...
                sys.exit('%s: ReplaceReadGroups failed: %s' % (e, os.path.join(output_dir, rg_old+'.bam')))
    ...
    for rg in _file.get('read_groups'):
        try:
            ...
            sys.exit('%s: AddCommentsToBam failed: %s' %(e, os.path.join(output_dir, rg.get('read_group_id')+'.bam')))
